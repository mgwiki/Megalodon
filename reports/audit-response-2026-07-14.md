...
## July 15 Increment: Source-Linked Clausal Core Seed
...
- The same generated source/certificate pair is committed as
  `tests/vampire_certificate/closed_cases/core.cnf.1.*`.
...
The tightened core audit now has one checked seed:

```text
TMPDIR=/project/tmp JOBS=10 MIN_CORE=1 RUN_CORE_CASES=1 \
  tests/vampire_certificate/run_native_cert_v1_core_closed_audit.sh
...
This is still not the audit's ten-proof milestone, and it is deliberately not
claimed as a THF/Megalodon-development success.  It proves a narrower but
necessary boundary condition: the clause-level native certificate path can be
source-linked and checked without preprocessing steps when Vampire starts from
already-clausal input.  The next step is to generate nine more nontrivial
clausal core seeds, then replace them with or extend them toward THF-exported
Megalodon obligations by adding a certified preprocessing layer rather than
folding preprocessing into the core count.
