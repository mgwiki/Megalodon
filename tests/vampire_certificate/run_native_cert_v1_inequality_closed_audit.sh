...
mkdir -p "$WORK_DIR"
ln -sfn "$WORK_DIR" "$TMPDIR/latest_native_cert_v1_inequality_closed_audit"

# This terminal gate tracks the current inequality-name/split edge case above
...
: > "$WORK_DIR/inequality_closed_cases.list"
...
while IFS= read -r native; do
  ...
  if [[ ! -s "$bad_file" ]] \
      && { grep -Fxq inequality_name_intro "$rules_file" || grep -Fxq inequality_split "$rules_file"; }; then
    printf '%s\tINEQUALITY_ELIGIBLE\t%s\n' "$base" "$(paste -sd, "$rules_file")" \
      >> "$WORK_DIR/eligible.tsv"
    printf '%s\n' "$base" >> "$WORK_DIR/inequality_closed_cases.list"
...
{
  printf 'INEQUALITY_ELIGIBLE %s\n' "$eligible_count"
  printf 'EXCLUDED_OR_LOWER_LAYER_ONLY %s\n' "$excluded_count"
  printf 'MIN_INEQUALITY %s\n' "$MIN_INEQUALITY"
} | tee "$WORK_DIR/counts.txt"

echo "native certificate v1 inequality closed audit artifacts: $WORK_DIR"
echo "native certificate v1 inequality closed audit latest link: $TMPDIR/latest_native_cert_v1_inequality_closed_audit"
echo "native certificate v1 inequality closed audit rule counts: $WORK_DIR/rule_counts.txt"
echo "native certificate v1 inequality closed audit excluded rule counts: $WORK_DIR/excluded_rule_counts.txt"
echo "native certificate v1 inequality closed audit first excluded rule counts: $WORK_DIR/first_excluded_rule_counts.txt"

if (( eligible_count < MIN_INEQUALITY )); then
  echo "inequality closed audit has fewer than $MIN_INEQUALITY inequality cases" >&2
