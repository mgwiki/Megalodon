...
WORK_DIR=${WORK_DIR:-"$(mktemp -d "$TMPDIR/native_cert_v1_definition_rewrite_closed_audit.XXXXXX")"}
MIN_DEFINITION_REWRITE=${MIN_DEFINITION_REWRITE:-1}
RUN_DEFINITION_REWRITE_CASES=${RUN_DEFINITION_REWRITE_CASES:-1}
JOBS=${JOBS:-7}

mkdir -p "$WORK_DIR"
ln -sfn "$WORK_DIR" "$TMPDIR/latest_native_cert_v1_definition_rewrite_closed_audit"

# This terminal gate tracks the current definition-rewrite-chain edge case above
# the AVATAR layer.  The committed corpus currently has one such closed case, so
# the default minimum is intentionally one rather than a broad milestone count.
allowed_rules=$(
  cat <<'RULES'
input
formula_input
...
: > "$WORK_DIR/definition_rewrite_closed_cases.list"
...
while IFS= read -r native; do
  ...
  if [[ ! -s "$bad_file" ]] && grep -Fxq definition_rewrite_chain "$rules_file"; then
    printf '%s\tDEFINITION_REWRITE_ELIGIBLE\t%s\n' "$base" "$(paste -sd, "$rules_file")" \
      >> "$WORK_DIR/eligible.tsv"
    printf '%s\n' "$base" >> "$WORK_DIR/definition_rewrite_closed_cases.list"
...
{
  printf 'DEFINITION_REWRITE_ELIGIBLE %s\n' "$eligible_count"
  printf 'EXCLUDED_OR_LOWER_LAYER_ONLY %s\n' "$excluded_count"
  printf 'MIN_DEFINITION_REWRITE %s\n' "$MIN_DEFINITION_REWRITE"
} | tee "$WORK_DIR/counts.txt"

echo "native certificate v1 definition-rewrite closed audit artifacts: $WORK_DIR"
echo "native certificate v1 definition-rewrite closed audit latest link: $TMPDIR/latest_native_cert_v1_definition_rewrite_closed_audit"
echo "native certificate v1 definition-rewrite closed audit rule counts: $WORK_DIR/rule_counts.txt"
echo "native certificate v1 definition-rewrite closed audit excluded rule counts: $WORK_DIR/excluded_rule_counts.txt"
echo "native certificate v1 definition-rewrite closed audit first excluded rule counts: $WORK_DIR/first_excluded_rule_counts.txt"

if (( eligible_count < MIN_DEFINITION_REWRITE )); then
  echo "definition-rewrite closed audit has fewer than $MIN_DEFINITION_REWRITE definition-rewrite cases" >&2
  sed -n '1,20p' "$WORK_DIR/first_excluded_rule_counts.txt" >&2
  exit 1
fi

if [[ "$RUN_DEFINITION_REWRITE_CASES" == "1" ]]; then
  CASE_LIST="$WORK_DIR/definition_rewrite_closed_cases.list" \
  WORK_DIR="$WORK_DIR/closed_check" \
  JOBS="$JOBS" \
    "$ROOT/tests/vampire_certificate/run_native_cert_v1_closed_corpus.sh"
  sed 's/\tCLOSED_PASS$/\tDEFINITION_REWRITE_CLOSED_PASS/' \
    "$WORK_DIR/closed_check/summary.tsv" > "$WORK_DIR/definition_rewrite_summary.tsv"
  awk -F '\t' '{count[$2]++} END {for (status in count) print status, count[status]}' \
    "$WORK_DIR/definition_rewrite_summary.tsv" \
    | sort > "$WORK_DIR/definition_rewrite_counts.txt"
  cat "$WORK_DIR/definition_rewrite_counts.txt"
fi
