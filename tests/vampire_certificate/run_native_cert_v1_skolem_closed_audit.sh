...
WORK_DIR=${WORK_DIR:-"$(mktemp -d "$TMPDIR/native_cert_v1_skolem_closed_audit.XXXXXX")"}
MIN_SKOLEM=${MIN_SKOLEM:-10}
RUN_SKOLEM_CASES=${RUN_SKOLEM_CASES:-1}
JOBS=${JOBS:-7}

mkdir -p "$WORK_DIR"
ln -sfn "$WORK_DIR" "$TMPDIR/latest_native_cert_v1_skolem_closed_audit"

# This gate tracks the explicit Skolemization layer above the certified THF
# preprocessing gate.  It permits the preprocessing fragment, one or more
# skolem_formula steps, and the same clausal rules.  It deliberately excludes
# AVATAR, introduced definitions, inequality splitting, and other broader macro
# steps.  A selected case must contain at least one skolem_formula step.
allowed_rules=$(
  cat <<'RULES'
input
formula_input
...
: > "$WORK_DIR/skolem_closed_cases.list"
...
while IFS= read -r native; do
  ...
  if [[ ! -s "$bad_file" ]] && grep -Fxq skolem_formula "$rules_file"; then
    printf '%s\tSKOLEM_ELIGIBLE\t%s\n' "$base" "$(paste -sd, "$rules_file")" \
      >> "$WORK_DIR/eligible.tsv"
    printf '%s\n' "$base" >> "$WORK_DIR/skolem_closed_cases.list"
...
{
  printf 'SKOLEM_ELIGIBLE %s\n' "$eligible_count"
  printf 'EXCLUDED_OR_LOWER_LAYER_ONLY %s\n' "$excluded_count"
  printf 'MIN_SKOLEM %s\n' "$MIN_SKOLEM"
} | tee "$WORK_DIR/counts.txt"

echo "native certificate v1 skolem closed audit artifacts: $WORK_DIR"
echo "native certificate v1 skolem closed audit latest link: $TMPDIR/latest_native_cert_v1_skolem_closed_audit"
echo "native certificate v1 skolem closed audit rule counts: $WORK_DIR/rule_counts.txt"
echo "native certificate v1 skolem closed audit excluded rule counts: $WORK_DIR/excluded_rule_counts.txt"
echo "native certificate v1 skolem closed audit first excluded rule counts: $WORK_DIR/first_excluded_rule_counts.txt"

if (( eligible_count < MIN_SKOLEM )); then
  echo "skolem closed audit has fewer than $MIN_SKOLEM skolemization cases" >&2
...
if [[ "$RUN_SKOLEM_CASES" == "1" ]]; then
  CASE_LIST="$WORK_DIR/skolem_closed_cases.list" \
  WORK_DIR="$WORK_DIR/closed_check" \
  JOBS="$JOBS" \
    "$ROOT/tests/vampire_certificate/run_native_cert_v1_closed_corpus.sh"
  sed 's/\tCLOSED_PASS$/\tSKOLEM_CLOSED_PASS/' \
    "$WORK_DIR/closed_check/summary.tsv" > "$WORK_DIR/skolem_summary.tsv"
  awk -F '\t' '{count[$2]++} END {for (status in count) print status, count[status]}' \
    "$WORK_DIR/skolem_summary.tsv" \
    | sort > "$WORK_DIR/skolem_counts.txt"
  cat "$WORK_DIR/skolem_counts.txt"
fi
