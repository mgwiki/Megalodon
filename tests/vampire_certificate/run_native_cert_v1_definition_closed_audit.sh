...
WORK_DIR=${WORK_DIR:-"$(mktemp -d "$TMPDIR/native_cert_v1_definition_closed_audit.XXXXXX")"}
MIN_DEFINITION=${MIN_DEFINITION:-10}
RUN_DEFINITION_CASES=${RUN_DEFINITION_CASES:-1}
JOBS=${JOBS:-7}

mkdir -p "$WORK_DIR"
ln -sfn "$WORK_DIR" "$TMPDIR/latest_native_cert_v1_definition_closed_audit"

# This gate tracks definition inputs above the explicit Skolemization layer.  It
# permits the lower certified layers plus definition_input facts that closed
# mode can justify without adding non-source premises.  It deliberately excludes
...
: > "$WORK_DIR/definition_closed_cases.list"
...
while IFS= read -r native; do
  ...
  if [[ ! -s "$bad_file" ]] && grep -Fxq definition_input "$rules_file"; then
    printf '%s\tDEFINITION_ELIGIBLE\t%s\n' "$base" "$(paste -sd, "$rules_file")" \
      >> "$WORK_DIR/eligible.tsv"
    printf '%s\n' "$base" >> "$WORK_DIR/definition_closed_cases.list"
...
{
  printf 'DEFINITION_ELIGIBLE %s\n' "$eligible_count"
  printf 'EXCLUDED_OR_LOWER_LAYER_ONLY %s\n' "$excluded_count"
  printf 'MIN_DEFINITION %s\n' "$MIN_DEFINITION"
} | tee "$WORK_DIR/counts.txt"

echo "native certificate v1 definition closed audit artifacts: $WORK_DIR"
echo "native certificate v1 definition closed audit latest link: $TMPDIR/latest_native_cert_v1_definition_closed_audit"
echo "native certificate v1 definition closed audit rule counts: $WORK_DIR/rule_counts.txt"
echo "native certificate v1 definition closed audit excluded rule counts: $WORK_DIR/excluded_rule_counts.txt"
echo "native certificate v1 definition closed audit first excluded rule counts: $WORK_DIR/first_excluded_rule_counts.txt"

if (( eligible_count < MIN_DEFINITION )); then
  echo "definition closed audit has fewer than $MIN_DEFINITION definition-input cases" >&2
...
if [[ "$RUN_DEFINITION_CASES" == "1" ]]; then
  CASE_LIST="$WORK_DIR/definition_closed_cases.list" \
  WORK_DIR="$WORK_DIR/closed_check" \
  JOBS="$JOBS" \
    "$ROOT/tests/vampire_certificate/run_native_cert_v1_closed_corpus.sh"
  sed 's/\tCLOSED_PASS$/\tDEFINITION_CLOSED_PASS/' \
    "$WORK_DIR/closed_check/summary.tsv" > "$WORK_DIR/definition_summary.tsv"
  awk -F '\t' '{count[$2]++} END {for (status in count) print status, count[status]}' \
    "$WORK_DIR/definition_summary.tsv" \
    | sort > "$WORK_DIR/definition_counts.txt"
  cat "$WORK_DIR/definition_counts.txt"
fi
