...
if [[ "${SOURCE_CONTEXT_ONLY:-0}" = "1" ]]; then
  ...
  bin/megalodon \
    -vampirecertv1sourcecontext \
    -vampirecertv1corepfcheck \
    -vampirecertv1 tests/vampire_certificate/native_cert_v1_source_map_local_set_reflexivity_valid.sexp \
    -vampirecertv1source tests/vampire_certificate/native_cert_v1_source_map_local_set_reflexivity_valid.th0.p \
    "$dummy" >"$WORK_DIR/native_cert_v1_source_map_local_set_reflexivity_corepf.log"

  if ! rg -q 'Vampire certificate v1 native core proof term checked 4 steps' \
      "$WORK_DIR/native_cert_v1_source_map_local_set_reflexivity_corepf.log"; then
    echo "focused source-context smoke did not prove a reflexive generated local set source" >&2
    exit 1
  fi
  if ! rg -q 'Vampire certificate v1 native core source assumptions remaining by kind known=1 local=0 definition=0 generated=0 conjecture=0 unresolved=0' \
      "$WORK_DIR/native_cert_v1_source_map_local_set_reflexivity_corepf.log"; then
    echo "focused source-context smoke did not classify remaining source assumptions after local set-reflexivity" >&2
    exit 1
  fi
  if ! rg -q 'source context issues count=1 sample=known_missing:c2:known:original_not_local_set_eq:eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' \
      "$WORK_DIR/native_cert_v1_source_map_local_set_reflexivity_corepf.log"; then
    echo "focused source-context smoke did not report the remaining known source issue after local set-reflexivity" >&2
    exit 1
  fi
...
bin/megalodon \
  -vampirecertv1preprocesspfcheck \
  -vampirecertv1 tests/vampire_certificate/native_cert_v1_equality_factoring_constraints_explicit_cross_core_pf_valid.sexp \
  -vampirecertv1source tests/vampire_certificate/native_cert_v1_equality_factoring_constraints_explicit_cross_core_pf_valid.th0.p \
  "$dummy" >"$WORK_DIR/native_cert_v1_equality_factoring_constraints_explicit_cross_core_pf_valid.log"

if ! rg -q 'Vampire certificate v1 native preprocess proof term checked 6 steps' \
    "$WORK_DIR/native_cert_v1_equality_factoring_constraints_explicit_cross_core_pf_valid.log"; then
  echo "native preprocess proof-term checker did not accept explicit crossed equality_factoring_constraints" >&2
  exit 1
fi
...
bin/megalodon \
  -vampirecertv1preprocesspfcheck \
  -vampirecertv1 tests/vampire_certificate/native_cert_v1_avatar_component_pf_valid.sexp \
  -vampirecertv1source tests/vampire_certificate/native_cert_v1_avatar_component_pf_valid.th0.p \
  "$dummy" >"$WORK_DIR/native_cert_v1_avatar_component_pf_valid.out" \
  2>"$WORK_DIR/native_cert_v1_avatar_component_pf_valid.err"

if ! rg -q 'Vampire certificate v1 native preprocess proof term checked 7 steps' \
    "$WORK_DIR/native_cert_v1_avatar_component_pf_valid.out"; then
  echo "native preprocess checker did not accept direct AVATAR definition/component proof terms" >&2
  exit 1
fi
...
if rg -q 'refuses certificate-derived Known primitive vampire_avatar_|refuses certificate-derived Known primitive vampire_resolve_|admit|aby|-allowincompleteqed' \
    "$WORK_DIR/native_cert_v1_avatar_component_pf_valid.out" \
    "$WORK_DIR/native_cert_v1_avatar_component_pf_valid.err"; then
  echo "native preprocess checker used a forbidden marker for direct AVATAR definition/component proof terms" >&2
  exit 1
fi

bin/megalodon \
  -vampirecertv1preprocesspfcheck \
  -vampirecertv1 tests/vampire_certificate/native_cert_v1_avatar_refutation_sat_resolution_pf_valid.sexp \
  -vampirecertv1source tests/vampire_certificate/native_cert_v1_avatar_refutation_sat_resolution_pf_valid.th0.p \
  "$dummy" >"$WORK_DIR/native_cert_v1_avatar_refutation_sat_resolution_pf_valid.out" \
  2>"$WORK_DIR/native_cert_v1_avatar_refutation_sat_resolution_pf_valid.err"

if ! rg -q 'Vampire certificate v1 native preprocess proof term checked 5 steps' \
    "$WORK_DIR/native_cert_v1_avatar_refutation_sat_resolution_pf_valid.out"; then
  echo "native preprocess checker did not accept AVATAR SAT resolution proof terms" >&2
  exit 1
fi

if rg -q 'refuses certificate-derived Known primitive vampire_avatar_|refuses certificate-derived Known primitive vampire_resolve_|admit|aby|-allowincompleteqed' \
    "$WORK_DIR/native_cert_v1_avatar_refutation_sat_resolution_pf_valid.out" \
    "$WORK_DIR/native_cert_v1_avatar_refutation_sat_resolution_pf_valid.err"; then
  echo "native preprocess checker used a forbidden marker for AVATAR SAT resolution proof terms" >&2
  exit 1
fi
...
bin/megalodon \
  ...
  "$dummy" >"$WORK_DIR/native_cert_v1_predicate_definition_fold_chain_pf_valid.out" \
  2>"$WORK_DIR/native_cert_v1_predicate_definition_fold_chain_pf_valid.err"

if ! rg -q 'Vampire certificate v1 native preprocess proof term checked 8 steps' \
    "$WORK_DIR/native_cert_v1_predicate_definition_fold_chain_pf_valid.out"; then
  echo "native preprocess checker did not accept predicate-definition fold-chain proof terms" >&2
  exit 1
fi

if rg -q 'refuses certificate-derived Known primitive vampire_predicate_definition_fold_chain_|admit|aby|-allowincompleteqed' \
    "$WORK_DIR/native_cert_v1_predicate_definition_fold_chain_pf_valid.out" \
    "$WORK_DIR/native_cert_v1_predicate_definition_fold_chain_pf_valid.err"; then
  echo "native preprocess checker used a forbidden marker for predicate-definition fold-chain proof terms" >&2
  exit 1
fi
...
if bin/megalodon \
  -vampirecertv1strict \
  -vampirecertv1 tests/vampire_certificate/native_cert_v1_primitive_expansion_requires_missing_bad.sexp \
  -vampirecertv1source tests/vampire_certificate/native_cert_v1_source_map_valid.th0.p \
  "$dummy" >"$WORK_DIR/native_cert_v1_primitive_expansion_requires_missing_bad.out" \
  2>"$WORK_DIR/native_cert_v1_primitive_expansion_requires_missing_bad.err"; then
  echo "strict native certificate v1 checker accepted primitive-expansion metadata without the singular required primitive" >&2
  exit 1
fi

if ! rg -q 'requires primitive_expansion_requires=resolve for kernel rule unit_resulting_resolution' \
    "$WORK_DIR/native_cert_v1_primitive_expansion_requires_missing_bad.err"; then
  echo "strict native certificate v1 primitive-expansion failure did not explain the missing singular required primitive" >&2
  exit 1
fi
...
bin/megalodon \
  ...
  -vampirecertv1 tests/vampire_certificate/native_cert_v1_source_map_local_set_reflexivity_valid.sexp \
  -vampirecertv1source tests/vampire_certificate/native_cert_v1_source_map_local_set_reflexivity_valid.th0.p \
  "$dummy" >"$WORK_DIR/native_cert_v1_source_map_local_set_reflexivity_valid.log"

if ! rg -q 'Vampire certificate v1 source map checked 2 sources' "$WORK_DIR/native_cert_v1_source_map_local_set_reflexivity_valid.log"; then
  echo "native certificate v1 source-map checker did not accept a reflexive local set source" >&2
  exit 1
fi

if ! rg -q 'source obligations audited total=2 .*set_reflexivity_checked=1' "$WORK_DIR/native_cert_v1_source_map_local_set_reflexivity_valid.log"; then
  echo "native certificate v1 source-obligation audit did not classify a reflexive local set source" >&2
  exit 1
fi
...
if ! rg -q 'Vampire certificate v1 native core proof term checked 4 steps' "$WORK_DIR/native_cert_v1_source_map_local_set_reflexivity_corepf.log"; then
  echo "native certificate v1 core proof-term checker did not prove a reflexive local set source" >&2
  exit 1
fi
if ! rg -q 'Vampire certificate v1 native core source assumptions remaining by kind known=1 local=0 definition=0 generated=0 conjecture=0 unresolved=0' "$WORK_DIR/native_cert_v1_source_map_local_set_reflexivity_corepf.log"; then
  echo "native certificate v1 core proof-term checker did not classify remaining source assumptions after local_set_reflexivity" >&2
  exit 1
fi
if ! rg -q 'source context issues count=1 sample=known_missing:c2:known:original_not_local_set_eq:eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' "$WORK_DIR/native_cert_v1_source_map_local_set_reflexivity_corepf.log"; then
  echo "native certificate v1 core proof-term checker did not report the remaining known source issue after local_set_reflexivity" >&2
  exit 1
fi
...
if bin/megalodon \
  ...
  -vampirecertv1source tests/vampire_certificate/native_cert_v1_kernel_skolemize_valid.th0.p \
  "$dummy" >"$WORK_DIR/native_cert_v1_kernel_skolemize_dependency_count_bad.out" \
  2>"$WORK_DIR/native_cert_v1_kernel_skolemize_dependency_count_bad.err"; then
  echo "strict native certificate v1 checker accepted bad Skolem dependency metadata" >&2
  exit 1
fi

if ! rg -q 'u1: strict certificate v1 kernel_v1 metadata field introduced_0_dependency_count expected 0 but got 1' \
    "$WORK_DIR/native_cert_v1_kernel_skolemize_dependency_count_bad.err"; then
  echo "strict native certificate v1 checker did not explain bad Skolem dependency metadata" >&2
  exit 1
fi
