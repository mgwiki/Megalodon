...
WORK_DIR=${WORK_DIR:-"$(mktemp -d "$TMPDIR/live_hammer_prefix4_failclosed_qualifying.XXXXXX")"}

mkdir -p "$WORK_DIR/out"
ln -sfn "$WORK_DIR" "$TMPDIR/latest_live_hammer_prefix4_failclosed_qualifying"
...
prefix_file="$WORK_DIR/hammer_prefix4_failclosed_qualifying.mg"
awk 'NR<=184 {print}' "$SOURCE_FILE" |
  sed -E 's/^aby(.*)\.$/vampire\1./' >"$prefix_file"

expected_successes_before_frontier=3
actual_vampire_commands=$(rg -c '^vampire( .*)?\.$' "$prefix_file" || true)
if [[ "$actual_vampire_commands" != "4" ]]; then
  echo "expected 4 generated vampire commands, got $actual_vampire_commands" >&2
  exit 1
fi
if awk '$0 !~ /^\/\// && ($0 ~ /(^|[^[:alnum:]_])(admit|aby)([^[:alnum:]_]|$)/ || $0 ~ /-allowincompleteqed/) {print FNR ":" $0}' \
    "$prefix_file" |
    rg . >&2; then
  echo "prefix4 fail-closed fixture contains an admission-shaped token" >&2
  exit 1
fi
...
set -e
if (( rc == 0 )); then
  echo "prefix4 unexpectedly passed; update this guard only after and3I has a small-kernel reconstruction" >&2
  exit 1
fi
...
echo "LIVE_HAMMER_PREFIX4_FAILCLOSED_QUALIFYING_PASS"
echo "live hammer prefix4 fail-closed qualifying artifacts: $WORK_DIR"
echo "live hammer prefix4 fail-closed qualifying latest link: $TMPDIR/latest_live_hammer_prefix4_failclosed_qualifying"
echo "live hammer prefix4 fail-closed qualifying vampire: $VAMPIRE"
